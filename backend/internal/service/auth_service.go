...
var (
	...
	ErrDisposableEmailNotAllowed = infraerrors.BadRequest("DISPOSABLE_EMAIL_NOT_ALLOWED", "disposable email addresses are not allowed")
...
type AuthService struct {
	...
	emailBlocklistRepo RegistrationEmailBlocklistRepository
...
func NewAuthService(
	...
	defaultSubAssigner DefaultSubscriptionAssigner,
	affiliateService *AffiliateService,
	emailBlocklistRepos ...RegistrationEmailBlocklistRepository,
) *AuthService {
	var emailBlocklistRepo RegistrationEmailBlocklistRepository
	if len(emailBlocklistRepos) > 0 {
		emailBlocklistRepo = emailBlocklistRepos[0]
	}
	return &AuthService{
		entClient:          entClient,
		userRepo:           userRepo,
		...
		cfg:                cfg,
		settingService:     settingService,
		emailBlocklistRepo: emailBlocklistRepo,
		emailService:       emailService,
		turnstileService:   turnstileService,
		emailQueueService:  emailQueueService,
		promoService:       promoService,
...
func (s *AuthService) validateRegistrationEmailPolicy(ctx context.Context, email string) error {
	if s.emailBlocklistRepo != nil {
		blocked, err := s.emailBlocklistRepo.IsRegistrationEmailBlocked(ctx, email)
		if err != nil {
			logger.LegacyPrintf("service.auth", "[Auth] Email blocklist lookup failed: %v", err)
			return ErrServiceUnavailable
		}
		if blocked {
